...
class ElasticsearchRAGSystem:
    ...
    def _generate_llm_response(self, question: str, context: str, results: List[Dict]) -> str:
        ...
        try:
            # Intentar usar Ollama si está configurado
...
Respuesta:"""
            ...
            # Fallback: respuesta inteligente del contexto
            else:
                return self._extract_smart_answer(question, context, results)
    ...
    def _extract_smart_answer(self, question: str, context: str, results: List[Dict]) -> str:
        ...
        # Buscar el artículo más relevante en los resultados
        ...
        for result in results:
            ...
            # Buscar artículo que contenga el tema detectado
            if detected_topic:
        ...
        # Si no se encontró artículo específico, usar el primer resultado
        ...
        # Generar respuesta según el tema
    ...
    def _format_piscina_answer(self, content: str, article: str) -> str:
        """Formatea respuesta sobre piscina"""
        ...
        # Buscar normas adicionales
        normas = []
        for line in content.split('\n'):
            line = line.strip()
            if line.startswith('•') and 'piscina' in line.lower():
                normas.append(line)
            elif line.startswith('•') and any(word in line.lower() for word in ['prohib', 'obligatorio', 'permitir']):
                if 'piscina' in content[:content.index(line)].lower()[-200:]:
                    normas.append(line)
    ...
    def _format_ruido_answer(self, content: str, article: str) -> str:
        """Formatea respuesta sobre ruidos"""
        response = "**Según la normativa de la comunidad:**\n\n"
        
        # Buscar horario de ruidos
        ...
        # Buscar normas adicionales sobre ruidos
        normas = []
        for line in content.split('\n'):
            line = line.strip()
    ...
    def _format_obra_answer(self, content: str, article: str) -> str:
        """Formatea respuesta sobre obras"""
        response = "**Según la normativa de la comunidad:**\n\n"
        
        # Buscar horarios de obras
        horarios = []
        for line in content.split('\n'):
            line = line.strip()
